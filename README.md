...
mkdir pasta
cd pasta
echo "conteúdo legal" > arquivo.txt
echo " e mais conteúdo" >> arquivo.txt
...
Reabra o programa e execute:

```text
cat /destino/pasta/arquivo.txt
rm /destino/pasta/atalho.txt
cat /destino/pasta/arquivo.txt
rmdir /destino/pasta
exit
```

O conteúdo deve persistir, a remoção do link deve preservar o arquivo e o `rmdir` deve recusar a pasta não vazia.
...
São 23 testes com subcasos, usando imagens temporárias. O disco da sua demonstração não é alterado. Os testes incluem blocos de fronteira, encadeamento, caminhos, links, proteção da árvore, persistência, compatibilidade, fim de entrada, i-nodes esgotados e disco realmente cheio. O teste de capacidade usa temporariamente cerca de 128 MiB em disco e algumas centenas de MiB de memória.

Para verificar integridade, a suíte percorre a árvore e confere tamanhos, referências `.`/`..`, i-nodes, blocos e mapas. Em falhas importantes, compara o SHA-256 da imagem inteira antes e depois.

Para novos testes em Python, prefira `executar("comando")`, que já abre a transação. Helpers internos que alteram estruturas devem ser chamados dentro de `with transacao():`.

## Entrega

Renomeie o ZIP com as matrículas reais, conforme o AVA: `NMatric1_NMatric2_NMatric3_NMatric4.zip`, usando apenas os integrantes do grupo. O pacote contém código, documentação e testes; `filesystem.img` é criada na primeira execução.
